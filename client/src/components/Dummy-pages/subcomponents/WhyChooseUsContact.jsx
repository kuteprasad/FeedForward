...
export default function WhyChooseUsContact() {
	return (
		<>
			...
			<section
				className="py-16 px-6 bg-[var(--section-bg)] 
			...
			>
				<div className="md:w-1/2 px-6 text-left">
					...
					<p className="text-[var(--text)] dark:text-[var(--text)] text-lg mb-8">
					...
					<div className="space-y-6">
						{[
							{
								icon: FaBolt,
								text: "Real-time Food Matching – AI-driven tech ensures fast & efficient food distribution.",
							},
							{
								icon: FaMapMarkerAlt,
								text: "Geo-location Based Connectivity – Matches donors & NGOs instantly.",
							},
							{
								icon: FaChartBar,
								text: "Data-Driven Insights – Track donations & impact effortlessly.",
							},
							{
								icon: FaUsers,
								text: "Community-Centered Approach – Strengthening bonds between food donors & NGOs.",
							},
						].map((item, index) => (
							<motion.div
								key={index}
							...
							>
								...
								<p className="text-[var(--text)]  text-lg">{item.text}</p>
							</motion.div>
						))}
					</div>
				</div>
			...
			{/* Contact Us Section */}
			<section
				className="py-16 px-6 bg-[var(--card-bg)] 
                         flex flex-col md:flex-row items-center transition-colors duration-300"
			>
				<div
					className="md:w-1/2 bg-[var(--section-bg)]
                      p-8 rounded-lg shadow-lg transition-colors duration-300"
				>
					<h3 className="text-2xl font-bold mb-4 text-[var(--text)] dark:text-[var(--text)]">
						Send Us a Message
					</h3>
					<form>
						...
						<textarea
							...
							className="w-full p-2 mb-4 bg-[var(--input-bg)] 
                       text-[var(--text)] dark:text-[var(--text)]
                       ...
                       transition-colors duration-300"
						...
						<motion.div whileHover={{ scale: 1.05 }}>
							<Button
								className="bg-[var(--primary-teal)] text-white px-6 py-2 rounded-lg 
					...
					<div className="space-y-6">
						{[
							...
							<div
								...
								className="flex items-center space-x-4"
							>
								<item.icon className="text-[var(--text)] text-2xl" />
								<p className="text-[var(--text-light)] text-lg">{item.text}</p>
							</div>
						))}
						<div className="flex space-x-4 mt-4">
							{[FaFacebook, FaInstagram, FaLinkedin].map((Icon, index) => (
								<Icon
									key={index}
									className="text-[var(--text)]  text-3xl 
